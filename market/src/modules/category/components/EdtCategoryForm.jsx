...
export const EdtCategoryForm = ({isOpen,setCategories, onclose, category}) => {
            ...
            Alert.fire({
                ...
                preConfirm: async () =>{
                    try {
                        ...
                        if(!response.error){
                            setCategories((categories) => [response.data,...categories.filter((it) => it.id !== values.id)]);
                            Alert.fire({
                                title:successTitle,
                                text:successMsj,
                                icon:"success",
    ...
    return <Modal backdrop="static" keyboard={false} show={isOpen} onHide={handleClose} >
        <Modal.Header closeButton >
            <Modal.Title>Actaliza Categoria</Modal.Title>
        </Modal.Header>
        <Modal.Body>
            <Form onSubmit={form.handleSubmit} >
                <Form.Group className='mb-3' >
                    <Form.Label>Nombre</Form.Label>
                    <FormControl name='name' placeholder='Calzado' value={form.values.name} onChange={form.handleChange} />
                    {
                        form.errors.name &&
